...
?>
    ...
    <div class="blog">
        ...
        <!-- blog text-->
			...
			<?php /*the_content('<a href="'. get_permalink($post->ID).'" class="small_button">'.__('read more','rt_theme').'</a>' );*/ ?>
		<?php 
			global $more;    // Declare global $more (before the loop).
			$more = 0;       // Set (inside the loop) to display content above the more tag.
			the_content("<a href='". get_permalink($post->ID)."' class='small_button'>Read more</a>");
		?>
        <!-- /blog text-->
        <?php endif;?>
